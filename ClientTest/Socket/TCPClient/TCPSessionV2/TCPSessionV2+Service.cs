...
public partial class TCPSessionV2
{
	...
	private async Task KeepAliveAsync(CancellationToken token)
	{
		...
		{
			Console.WriteLine("KeepAliveAsync canceled");
		}
		catch (Exception e)
		{
			Console.WriteLine($"KeepAlive Error: {e.Message}");
		}
	}

	private void _KeepAliveTick()
	{
		...
		{
			Console.WriteLine($"Failed to send Ping: {e.Message}");
